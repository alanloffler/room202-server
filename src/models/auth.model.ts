...
export class AuthModel {
  static async createUser(userData: IUser): Promise<ResultSetHeader[]> {
    const user = {
      name: userData.name,
  ...
  static async emailUniqueCheck(email: string): Promise<boolean> {
    const connection = await pool.getConnection();
    const [rows] = await connection.execute<RowDataPacket[]>('SELECT * FROM users WHERE email=?', [email]);
    connection.release();
  ...
  static async validatePassword(passwordReq: string, passwordDb: string): Promise<boolean> {
  ...
  static async generateToken(id: number | undefined): Promise<string> {
    return jwt.sign({ id: id }, process.env.TOKEN_SECRET || 'tokenroom202', { expiresIn: 60 * 60 * 24 });
  }

  static async tokenExpired(token: string): Promise<boolean> {
